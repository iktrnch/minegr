...
Concurrent clients may use `start`, `stop`, `restart`, `status`, `logs`, and `console`. Multiple console clients share one command FIFO.
## Workflow
...
## Rules
- Every command handshakes with the Minegr version, server UUID, and canonical configuration path. Stop-mode handshakes permit a version mismatch.
...
- Stop finishes active backup recovery, drains accepted console commands, cancels restart startup, waits 60 seconds for Java, then escalates to `SIGTERM` and `SIGKILL` after another 10 seconds. New work is rejected once stop is accepted.
